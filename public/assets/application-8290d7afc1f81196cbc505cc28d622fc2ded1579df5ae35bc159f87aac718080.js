...
(function() {
  var judge_winner, reset_winner, set_winner;
  ...
  $(document).on('turbolinks:load', function() {
    ...
    if ($('body').data('controller') === 'tournaments' && $('body').data('action') === 'edit_game') {
      ...
      $('.game_game_records_score input').change(function() {
        ...
        if (winner !== null) {
          set_winner(winner);
          return $('#winner-select').val(winner);
        } else {
          return $('#winner-select').val('');
        }
      });
...
(function() {
  $(document).on('turbolinks:load', function() {
    ...
    if ($('body').data('controller') === 'tournaments' && $('body').data('action').match(/players/)) {
      setTeamsCount = function() {
        var teams_count;
        teams_count = $.trim($('#tournament_teams_text').val()).split(/\r|\n/).length;
        $('#teams_count').text(teams_count);
        if (teams_count !== Number($('#tournament_size').text())) {
          return $('#teams_count').addClass('text-danger');
        } else {
          return $('#teams_count').removeClass('text-danger');
        }
      };
      setTeamsCount();
      return $('#tournament_teams_text').on('input propertychange', function() {
        return setTeamsCount();
      });
    }
  });
...
(function() {
  $(document).on('turbolinks:load', function() {
    if ($('body').data('controller') === 'tournaments' && $('body').data('action') === 'show') {
      ...
      $("#btnImageDownload").button('loading');
      setTimeout(function() {
        return html2canvas($(".bracket"), {
          ...
          onrendered: function(canvas) {
            ...
            $("#btnImageDownload").button('reset');
            return $(".bracket").css('overflow-x', 'scroll').css('overflow-y', 'scroll').css('width', '100%').css('padding-right', '0');
          }
        });
      }, 3000);
    }
...
(function() {
  var bindTwitterEventHandlers, loadTwitterSDK, renderTweetButtons, twttr_events_bound;

  twttr_events_bound = false;

  $(function() {
    return loadTwitterSDK(function() {
  ...
  loadTwitterSDK = function(callback) {
    return $.getScript('//platform.twitter.com/widgets.js', callback);
  };
